...
function createMessageFragment(msg, prevMsg, nextMsg, lastSenderRef) {
    ...
    const groupMember = null;

    if (settings.inChatAvatarEnabled) {
        const isSameSenderNormal = msg.sender === lastSenderRef.current;
        const shouldHide = !settings.alwaysShowAvatar && isSameSenderNormal;
        if (shouldHide) {
    ...
    if (!groupMember && msg.sender !== 'user' && msg.sender !== null && (settings.showPartnerNameInChat || showPartnerNameInChat)) {
