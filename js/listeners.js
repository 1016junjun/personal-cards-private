...
function _updateSplashSlide() {
    ...
    slides.forEach(function(s, i) {
        ...
        if (i < splashCurrent) s.classList.add('exit-left');
    });

    var currentSlide = document.querySelector('.splash-slide[data-index="' + splashCurrent + '"]');
    if (currentSlide) currentSlide.classList.add('active');

    dots.forEach(function(d, i) {
        d.classList.remove('active', 'done');
        if (i < splashCurrent) d.classList.add('done');
        if (i === splashCurrent) d.classList.add('active');
    });

    if (prevBtn) prevBtn.disabled = splashCurrent === 0;
    if (pageNum) pageNum.textContent = (splashCurrent + 1) + ' / ' + splashTotal;

    if (splashCurrent === splashTotal - 1) {
...
function _setupSplashDots() {
    var dotsContainer = document.getElementById('splash-dots');
    if (!dotsContainer) return;
    dotsContainer.innerHTML = '';
    for (var i = 0; i < splashTotal; i++) {
...
function _handlePledgeInput() {
    var input = document.getElementById('splash-pledge-input');
    var hint = document.getElementById('splash-pledge-hint');
    var enterBtn = document.getElementById('splash-enter-btn');
    if (!input) return;
    input.addEventListener('input', function() {
        var required = document.getElementById('splash-pledge-text');
        var requiredText = required ? required.textContent.trim() : '';
        var entered = input.value.trim();
        var isMatch = entered === requiredText;
        if (hint) hint.style.color = isMatch ? 'rgba(197,164,126,0.7)' : 'rgba(255,80,80,0.7)';
        if (enterBtn) enterBtn.classList.toggle('ready', isMatch);
    });
}

function _handleSplashEnter() {
    var input = document.getElementById('splash-pledge-input');
    var required = document.getElementById('splash-pledge-text');
    if (!input || !required) return;
    if (input.value.trim() !== required.textContent.trim()) {
    ...
    var splashEl = document.getElementById('splash-declaration');
    if (splashEl) {
        splashEl.style.transition = 'opacity 0.6s ease, transform 0.6s ease';
        splashEl.style.opacity = '0';
        splashEl.style.transform = 'scale(0.95)';
        setTimeout(function() { splashEl.style.display = 'none'; }, 600);
    }
    startTourIfNeeded();
}
...
function setupTutorialListeners() {
    ...
    var prevBtn = document.getElementById('splash-prev-btn');
    var enterBtn = document.getElementById('splash-enter-btn');
    var splashDotsContainer = document.getElementById('splash-dots');
    ...
    if (enterBtn) {
        enterBtn.addEventListener('click', _handleSplashEnter);
    }

    if (splashDotsContainer) {
        splashDotsContainer.addEventListener('click', function(e) {
...
function _updateTourStep() {
    ...
    if (!tourSteps.length) {
        tourSteps = [
            { title: '欢迎使用', content: '这是我们的专属空间，在这里可以和我聊天、记录心情、占卜运势等。' },
            { title: '发送消息', content: '在底部输入框输入文字，点击发送按钮或按 Enter 键发送消息。' },
            { title: '更多功能', content: '点击右上角的设置按钮，可以探索更多自定义选项和功能。' },
        ];
    }

    if (titleEl) titleEl.textContent = tourSteps[tourIndex].title;
    if (contentEl) contentEl.textContent = tourSteps[tourIndex].content;
    if (counterEl) counterEl.textContent = (tourIndex + 1) + ' / ' + tourSteps.length;
    if (prevBtn) prevBtn.style.visibility = tourIndex === 0 ? 'hidden' : 'visible';
    if (nextBtn) nextBtn.textContent = tourIndex === tourSteps.length - 1 ? '完成' : '下一步';
}
